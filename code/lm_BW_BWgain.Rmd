...
fit = lm(Weight_g ~ TimePoint_Weeks + BirthMode + Sex, data = BW_wk)
summary(fit)
allEffects(fit)
...
fit_lme4 = lmer(Weight_g ~ TimePoint_Weeks + BirthMode + Sex + (TimePoint_Weeks|FamilyID), data = BW_wk)
summary(fit_lme4)
allEffects(fit_lme4)
fit_lme4_f = lmer(Weight_g ~ TimePoint_Weeks + BirthMode + (TimePoint_Weeks|FamilyID), data = BW_wk %>% filter(Sex == "F"))
summary(fit_lme4_f)
allEffects(fit_lme4_f)
fit_lme4_m = lmer(Weight_g ~ TimePoint_Weeks + BirthMode + (TimePoint_Weeks|FamilyID), data = BW_wk %>% filter(Sex == "M"))
summary(fit_lme4_m)
allEffects(fit_lme4_m)

fit_nlme = lme(Weight_g ~ TimePoint_Weeks + BirthMode + Sex, random = ~1|FamilyID, data = BW_wk, method = "REML")
summary(fit_nlme)
allEffects(fit_nlme)
fit_nlme_f = lme(Weight_g ~ TimePoint_Weeks + BirthMode, random = ~1|FamilyID, data = BW_wk %>% filter(Sex == "F"), method = "REML")
summary(fit_nlme_f)
allEffects(fit_nlme_f)
fit_nlme_m = lme(Weight_g ~ TimePoint_Weeks + BirthMode, random = ~1|FamilyID, data = BW_wk %>% filter(Sex == "M"), method = "REML")
summary(fit_nlme_m)
allEffects(fit_nlme_m)
```
